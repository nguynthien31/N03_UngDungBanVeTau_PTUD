...
/**
 * Tab Quản lý Giá vé & Hạng vé tàu. Chức năng: Thêm, cập nhật, xóa, tìm kiếm
 * giá vé theo tuyến và loại ghế. Hỗ trợ điều chỉnh giá theo mùa cao điểm.
 */
public class TAB_Gia extends JPanel {
	...
	// =========================================================================
	private static final String[] COLS = { "Mã giá vé", "Ga đi", "Ga đến", "Loại ghế", "Giá vé (VNĐ)", "Mùa cao điểm" };
	...
	private static final String[] LOAI_GHE = { "Ghế ngồi mềm", "Ghế ngồi cứng", "Nằm mềm điều hòa",
	...
	private void timKiemGiaVe() {
		...
		JOptionPane.showMessageDialog(this, "Tìm kiếm: Ga đi = \"" + gaDi + "\", Ga đến = \"" + gaDen
				+ "\", Loại ghế = \"" + loaiGhe + "\"\n(Chưa kết nối dữ liệu)", "Thông báo",
	...
	private void moDialogThem() {
		...
		JPanel form = buildForm(
				new String[] { "Mã giá vé:", "Ga đi:", "Ga đến:", "Loại ghế:", "Giá vé (VNĐ):", "Mùa cao điểm:" },
				new JComponent[] { txtMaGiaVe, cbGaDi, cbGaDen, cbLoaiGhe, txtGiaVe, cbCaoDiem });

		JButton btnLuu = makeBtn("Lưu", BtnStyle.PRIMARY);
		btnLuu.addActionListener(e -> {
			if (kiemTraRong(txtMaGiaVe, txtGiaVe) && kiemTraSo(txtGiaVe)) {
				tableModel.addRow(new Object[] { txtMaGiaVe.getText().trim(), cbGaDi.getSelectedItem(),
						cbGaDen.getSelectedItem(), cbLoaiGhe.getSelectedItem(), formatGia(txtGiaVe.getText().trim()),
	...
	private void moDialogCapNhat() {
		...
		if (row < 0) {
			JOptionPane.showMessageDialog(this, "Vui lòng chọn một giá vé cần cập nhật.", "Chưa chọn dòng",
		...
		JComboBox<String> cbGaDen = makeComboBox(TINH_THANH, false);
		JComboBox<String> cbLoaiGhe = makeComboBox(LOAI_GHE, false);
		JTextField txtGiaVe = makeFieldWithValue(tableModel.getValueAt(row, 4).toString().replaceAll("[^0-9]", ""));
		JComboBox<String> cbCaoDiem = makeComboBox(MUA_CAO_DIEM, false);
	...
	private void xoaGiaVe() {
		int row = table.getSelectedRow();
		if (row < 0) {
			JOptionPane.showMessageDialog(this, "Vui lòng chọn một giá vé cần xóa.", "Chưa chọn dòng",
					JOptionPane.WARNING_MESSAGE);
			return;
		}
		String ma = tableModel.getValueAt(row, 0).toString();
		int confirm = JOptionPane.showConfirmDialog(this, "Bạn có chắc muốn xóa giá vé \"" + ma + "\" không?",
	...
	private boolean kiemTraSo(JTextField field) {
		try {
			...
			JOptionPane.showMessageDialog(this, "Giá vé phải là số nguyên dương (VD: 500000).", "Giá trị không hợp lệ",
					JOptionPane.WARNING_MESSAGE);
			field.requestFocus();
			return false;
		}
	...
	/**
	 * Định dạng số giá vé có dấu phân cách hàng nghìn. VD: "500000" → "500.000"
	 */
	...
	/**
	 * Tạo nút theo style chỉ định. PRIMARY → xanh | SECONDARY → xám | DANGER → đỏ
	 */

...
/**
 * Tab Quản lý Lịch trình & Chuyến tàu. Chức năng: Thêm, cập nhật, xóa, tìm kiếm
 * chuyến tàu. Trường: Mã chuyến, Tên tàu, Ga đi, Ga đến, Ngày KH, Giờ KH, Giờ
 * cập bến, Trạng thái.
 */
public class TAB_LichTrinh_ChuyenTau extends JPanel {
	...
	// CỘT BẢNG – CỐ ĐỊNH
	// =========================================================================
	private static final String[] COLS = { "Mã chuyến", "Tên tàu", "Ga đi", "Ga đến", "Ngày KH", "Giờ khởi hành",
			"Giờ cập bến", "Trạng thái" };

	// =========================================================================
	// DANH SÁCH TỈNH/THÀNH PHỐ VIỆT NAM (63 tỉnh thành)
	// =========================================================================
	private static final String[] TINH_THANH = { "An Giang", "Bà Rịa - Vũng Tàu", "Bắc Giang", "Bắc Kạn", "Bạc Liêu",
			"Bắc Ninh", "Bến Tre", "Bình Định", "Bình Dương", "Bình Phước", "Bình Thuận", "Cà Mau", "Cần Thơ",
			"Cao Bằng", "Đà Nẵng", "Đắk Lắk", "Đắk Nông", "Điện Biên", "Đồng Nai", "Đồng Tháp", "Gia Lai", "Hà Giang",
			"Hà Nam", "Hà Nội", "Hà Tĩnh", "Hải Dương", "Hải Phòng", "Hậu Giang", "Hòa Bình", "Hưng Yên", "Khánh Hòa",
	...
	// =========================================================================
	private static final String[] TRANG_THAI = { "Chờ khởi hành", "Đúng giờ", "Trễ", "Hủy" };
	...
	private JPanel buildFilterCard() {
		...
		// ComboBox tự co theo nội dung như TAB_Gia, DatePicker đủ rộng hiển thị
		// dd/MM/yyyy
	...
	private JPanel buildTableBody() {
		...
		JLabel lblEmpty = new JLabel("Chưa có dữ liệu – nhấn \"+ Thêm mới\" hoặc tìm kiếm", SwingConstants.CENTER);
	...
	private void moDialogThem() {
		...
		JPanel form = buildForm(
				new String[] { "Mã chuyến:", "Tên tàu:", "Ga đi:", "Ga đến:", "Ngày khởi hành:", "Giờ khởi hành:",
						"Giờ cập bến:", "Trạng thái:" },
				new JComponent[] { txtMaChuyen, txtTenTau, cbGaDi, cbGaDen, spNgayKH, cbGioKH, cbGioCapBen,
						cbTrangThai });

		JButton btnLuu = makeBtn("Lưu", BtnStyle.PRIMARY);
		btnLuu.addActionListener(e -> {
			if (kiemTraRong(txtMaChuyen, txtTenTau)) {
				tableModel.addRow(new Object[] { txtMaChuyen.getText().trim(), txtTenTau.getText().trim(),
						cbGaDi.getSelectedItem(), cbGaDen.getSelectedItem(), spNgayKH.getSelectedDate(),
						cbGioKH.getSelectedItem(), cbGioCapBen.getSelectedItem(), cbTrangThai.getSelectedItem() });
				dialog.dispose();
			}
		});
	...
	private void moDialogCapNhat() {
		...
		if (row < 0) {
			JOptionPane.showMessageDialog(this, "Vui lòng chọn một chuyến tàu cần cập nhật.", "Chưa chọn dòng",
		...
		btnLuu.addActionListener(e -> {
			if (kiemTraRong(txtTenTau)) {
				...
				tableModel.setValueAt(spNgayKH.getSelectedDate(), row, 4);
	...
	private void xoaChuyenTau() {
		int row = table.getSelectedRow();
		if (row < 0) {
			JOptionPane.showMessageDialog(this, "Vui lòng chọn một chuyến tàu cần xóa.", "Chưa chọn dòng",
					JOptionPane.WARNING_MESSAGE);
			return;
		}
		String maChuyen = tableModel.getValueAt(row, 0).toString();
		int confirm = JOptionPane.showConfirmDialog(this, "Bạn có chắc muốn xóa chuyến \"" + maChuyen + "\" không?",
	...
	private void applyFieldStyle(JTextField tf) {
		...
		tf.addFocusListener(new java.awt.event.FocusAdapter() {
			...
			public void focusGained(java.awt.event.FocusEvent e) {
				tf.setBorder(BorderFactory.createCompoundBorder(new LineBorder(ACCENT_FOC, 2, true),
	...
	/**
	 * Tạo nút theo style. PRIMARY → xanh | SECONDARY → xám | DANGER → đỏ
	 */
	...
	/**
	 * Ô nhập ngày có popup lịch. - Click vào ô → hiện popup lịch tháng. - Chọn
	 * tháng/năm bằng ComboBox, click vào ngày để chọn. - Trả kết quả qua
	 * getSelectedDate() dạng dd/MM/yyyy.
	 */
	private static class DatePickerField extends JPanel {
		...
		DatePickerField(String existingValue) {
			...
			txtHienThi = new JTextField(
					existingValue.isEmpty() ? new SimpleDateFormat(DATE_FORMAT).format(calendar.getTime())
							: existingValue);
			...
			txtHienThi.setBorder(BorderFactory.createCompoundBorder(new LineBorder(BORDER, 1, true),
		...
		private void hienPopup() {
			...
			lichPanel.setBorder(BorderFactory.createCompoundBorder(new LineBorder(BORDER, 1),
		...
		private void dienLich() {
			...
			for (int ngay = 1; ngay <= soNgay; ngay++) {
				...
				btnNgay.setForeground(laChon ? Color.WHITE : laHomNay ? ACCENT : TEXT_DARK);
